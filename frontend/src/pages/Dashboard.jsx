...
function toIsoWithClientTime(dateValue) {
  ...
  if (typeof dateValue === "string" && dateValue.length === 10) {
    ...
    const combined = new Date(`${dateValue}T${hhmmss}`); // Combine the input date with the current time
    return combined.toISOString();
  }

  //If a complete date/time is provided
  try {
...
const Dashboard = () => {
  ...
  //filters transactions based on the selected time period (like weekly, monthly, yearly)
  ...
  // Filters and keeps only the transactions in the current time frame.
  ...
  const displayedExpense = showAllExpense 
    ...
    // This function fetches the dashboard data from the backend, 
    // formats the response into a frontend-friendly structure, 
    // stores it in overviewMeta, and manages the loading state while the request is in progress.
  ...
  // add/ edit or delete
  //This function validates the form, sends the new transaction to the appropriate API (income or expense), 
  // refreshes the dashboard data, resets the form, 
  // closes the modal, and handles loading and errors.
  const handleAddTransaction = async () => {
    ...
    <div className={dashboardStyles.container}>
      ...
      <div className={dashboardStyles.summaryGrid}>
        ...
        additionalContent={
          <div className='flex item-center gap-2 mt-2 text-sm'>
